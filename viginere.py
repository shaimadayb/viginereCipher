...
def cesar_cipher (message, key) :
    ...
    for char in message :
        ...
        index_carac_in_printable = string.printable.index(char)
        index_crypted_char = (index_carac_in_printable + key)%len(string.printable)
        crypted_char = string.printable[index_crypted_char]
...
crypted_message = cesar_cipher("salut les bgs", 13)
...
def brute_force_cesar_cipher(crypted_message):
	for possible_key in range(0, len(alphabet)):
		print(cesar_cipher(crypted_message, possible_key))
...
def is_prime(number):
	if type(number) != int:
		raise("the imput is not integer")
	elif number < 2:
		return False
...
if number == 5142:
	...
	print("nous avons réussi à finir la boucle for")
